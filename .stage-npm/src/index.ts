...
import { randomProblem } from './luogu/api.ts'
import { LANGUAGES, LUOGU_ORIGIN, VERDICTS } from './luogu/constants.ts'
...
import { DIFFICULTY } from './luogu-types.ts'
...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      const client = newClient(exec.signal)
      const pick = await randomProblem(client, {
        type: args.type,
        difficulty: args.difficulty,
        keyword: args.keyword,
      })
