...
/**
 * Locate the data section in a payload. Lentille nests under `data.data` for
 * problem detail; legacy nests under `currentData.problem`. `keys` offers an
 * ordered descent list; empty means "the whole data section".
 */
function locateSection(payload: unknown, keys: string[]): UnknownRecord | undefined {
  let node = payload
  // Enter data/currentData envelope.
  if (isRecord(node)) {
    if (isRecord(node['data'])) node = node['data']
    else if (isRecord(node['currentData'])) node = node['currentData']
